function calculate() {
    ...
    if ($( "#myform" ).valid()) {
        ...
        /* if the operator was "Add" then add the numbers */
        if (operator == "Add") {
            result = operand1fp + operand2fp;
        }
 
        /* if the operator was "Sub" then subtract the numbers*/
        if (operator == "Sub") {
            result = operand1fp - operand2fp;
        }

        /* if operator was "Mult" the multiply the numbers */
        if (operator == "Mult") {
            result = operand1fp * operand2fp;
        }
        /* if operator was "Div" the divide the numbers */
        if (operator == "Div") {
            result = operand1fp / operand2fp;
        }
